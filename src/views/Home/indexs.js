...
// 挂载全局 要引入 const { $http } = React 发送请求
const { $http } = React
//表格数据  UserColumns  TaskColumns
const UserColumns = [
  {
    title: '用户名',
    dataIndex: 'username',
    ...
    return (
      <div className={style['page-home']}>
        <div className="header-wrapper">
          <Row gutter={16}>
            { /*  数据map       Space 间距           headerBanner[item.aliasCurrency]   报错                                  */}
        ...
        <div className="home-chart mt-20">
          <Card title='访问量' extra={
            <Radio.Group size="small" className="fs-12" value={this.state.date} onChange={this.handleYearChange}>
              ...
              <Radio.Button value='2'>去年</Radio.Button>
            </Radio.Group>
          }><Row gutter={16}>
              <Col span={16}>
                {/* style = {{ height:'330px'}} */}
              ...
              <Col span={8}>
                <div className="p15">
                  {/* `pb-10 fs-20 text-grey-6 ${style['chart-title']}` 字符串拼接  */}
                  <p className={`pb-10 fs-20 text-grey-6 ${style['chart-title']}`}>上月访问</p>
                  <p>同上期增长</p>
                  <Progress percent={chartData.monthVisit} strokeWidth={12}></Progress>
                </div>
                <div className="p15">
                  {/* `pb-10 fs-20 text-grey-6 ${style['chart-title']}` 字符串拼接  */}
                  <p className={`pb-10 fs-20 text-grey-6 ${style['chart-title']}`}>月下载数</p>
                  <p>同上期增长</p>
                  <Progress percent={chartData.monthDownload} strokeWidth={12}></Progress>
                </div>
                <div className="p15">
                  {/* `pb-10 fs-20 text-grey-6 ${style['chart-title']}` 字符串拼接  */}
                  <p className={`pb-10 fs-20 text-grey-6 ${style['chart-title']}`}>月收入</p>
                  <p>同上期增长</p>
                  <Progress percent={chartData.monthIncome} strokeWidth={12}></Progress>
                </div>
              </Col>
            </Row>
          </Card>
